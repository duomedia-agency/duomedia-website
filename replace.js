const fs = require('fs');

let html = fs.readFileSync('admin.html', 'utf8');
const startIndex = html.indexOf('    <!-- ========== SCRIPTS ========== -->');

if (startIndex !== -1) {
    html = html.substring(0, startIndex);
}

const newScript = `    <!-- ========== SCRIPTS ========== -->
<script>
// ============ CONFIGURATION ============
const REPO_OWNER = 'duomedia-agency';
const REPO_NAME = 'DuoMedia_Website';
const BRANCH = 'main';

let githubToken = localStorage.getItem('github_token') || '';
let siteData = { portfolio: [], brands: [], testimonials: [] };
let dataFileSha = '';

// ============ ELEMENTS ============
const gate = document.getElementById('admin-gate');
const dashboard = document.getElementById('admin-dashboard');
const logoutBtn = document.getElementById('logout-btn');
const authError = document.getElementById('auth-error');

// Replace login inputs
document.getElementById('auth-email').style.display = 'none';
const pwInput = document.getElementById('auth-password');
pwInput.placeholder = "GitHub Personal Access Token";
pwInput.type = "password";

// ============ AUTH ============
document.getElementById('auth-btn').addEventListener('click', doLogin);
pwInput.addEventListener('keydown', e => { if (e.key === 'Enter') doLogin(); });

async function doLogin() {
    const token = pwInput.value.trim();
    if (!token) return;
    
    // Verify token by trying to fetch the repository
    try {
        const res = await fetch(\`https://api.github.com/repos/\${REPO_OWNER}/\${REPO_NAME}\`, {
            headers: { 'Authorization': \`token \${token}\` }
        });
        
        if (!res.ok) throw new Error('Token non valido o permessi insufficienti');
        
        githubToken = token;
        localStorage.setItem('github_token', token);
        showDashboard();
    } catch (err) {
        authError.textContent = err.message;
        authError.style.display = 'block';
        setTimeout(() => { authError.style.display = 'none'; }, 3000);
    }
}

logoutBtn.addEventListener('click', () => {
    localStorage.removeItem('github_token');
    githubToken = '';
    showGate();
});

function showGate() {
    gate.style.display = '';
    dashboard.classList.remove('visible');
    logoutBtn.classList.remove('visible');
    pwInput.value = '';
}

async function showDashboard() {
    gate.style.display = 'none';
    dashboard.classList.add('visible');
    logoutBtn.classList.add('visible');
    await loadDataFromGitHub();
    renderAll();
}

// Auto-login if token exists
if (githubToken) {
    showDashboard();
}

// ============ GITHUB API HELPERS ============
async function fetchWithToken(url, options = {}) {
    const defaultHeaders = {
        'Authorization': \`token \${githubToken}\`,
        'Accept': 'application/vnd.github.v3+json',
        'Content-Type': 'application/json'
    };
    options.headers = { ...defaultHeaders, ...options.headers };
    const res = await fetch(url, options);
    if (!res.ok) {
        if (res.status === 401) {
            alert("Token scaduto o non valido. Fai il login di nuovo.");
            logoutBtn.click();
        }
        throw new Error(\`GitHub API Error: \${res.statusText}\`);
    }
    return res.json();
}

// Convert string to base64 keeping unicode (utf-8)
function b64EncodeUnicode(str) {
    return btoa(encodeURIComponent(str).replace(/%([0-9A-F]{2})/g,
        function toSolidBytes(match, p1) {
            return String.fromCharCode('0x' + p1);
    }));
}

async function loadDataFromGitHub() {
    try {
        // Fetch the file from GitHub API to get the content and SHA
        const url = \`https://api.github.com/repos/\${REPO_OWNER}/\${REPO_NAME}/contents/data.json?ref=\${BRANCH}\`;
        
        const response = await fetch(url, { headers: { 'Authorization': \`token \${githubToken}\` } });
        
        if (response.status === 404) {
            // File doesn't exist yet, we will create it on first save
            siteData = { portfolio: [], brands: [], testimonials: [] };
            dataFileSha = '';
            return;
        }
        
        if (!response.ok) throw new Error('Impossibile caricare data.json');
        
        const fileData = await response.json();
        dataFileSha = fileData.sha;
        
        // Content is base64 encoded
        const jsonString = decodeURIComponent(atob(fileData.content).split('').map(function(c) {
            return '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2);
        }).join(''));
        
        siteData = JSON.parse(jsonString);
        
        // Ensure arrays exist
        if (!siteData.portfolio) siteData.portfolio = [];
        if (!siteData.brands) siteData.brands = [];
        if (!siteData.testimonials) siteData.testimonials = [];
        
    } catch (err) {
        console.error("Error loading data:", err);
        alert("Errore nel caricamento dei dati: " + err.message);
    }
}

async function saveDataToGitHub() {
    const jsonString = JSON.stringify(siteData, null, 2);
    const contentBase64 = b64EncodeUnicode(jsonString);
    
    const url = \`https://api.github.com/repos/\${REPO_OWNER}/\${REPO_NAME}/contents/data.json\`;
    
    const body = {
        message: 'Aggiornamento dati (via admin panel)',
        content: contentBase64,
        branch: BRANCH
    };
    
    if (dataFileSha) {
        body.sha = dataFileSha;
    }
    
    const res = await fetchWithToken(url, {
        method: 'PUT',
        body: JSON.stringify(body)
    });
    
    // Update SHA for future saves
    dataFileSha = res.content.sha;
}

// Convert file to base64
function getBase64(file) {
    return new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.readAsDataURL(file);
        reader.onload = () => {
            // Remove the data:image/jpeg;base64, prefix
            const base64 = reader.result.split(',')[1];
            resolve(base64);
        };
        reader.onerror = error => reject(error);
    });
}

async function uploadFileToGitHub(file, folder, progressBar) {
    const fileName = Date.now() + '_' + file.name.replace(/[^a-zA-Z0-9._-]/g, '');
    const path = \`assets/uploads/\${folder}/\${fileName}\`;
    
    if (progressBar) progressBar.parentElement.classList.add('active');
    
    const base64Content = await getBase64(file);
    
    const url = \`https://api.github.com/repos/\${REPO_OWNER}/\${REPO_NAME}/contents/\${path}\`;
    
    try {
        if (progressBar) progressBar.style.width = '50%';
        
        await fetchWithToken(url, {
            method: 'PUT',
            body: JSON.stringify({
                message: \`Upload immagine \${fileName}\`,
                content: base64Content,
                branch: BRANCH
            })
        });
        
        if (progressBar) {
            progressBar.style.width = '100%';
            setTimeout(() => { 
                progressBar.parentElement.classList.remove('active'); 
                progressBar.style.width = '0%'; 
            }, 600);
        }
        
        // Return the GitHub Pages URL for the image
        return \`./\${path}\`;
        
    } catch (err) {
        if (progressBar) progressBar.parentElement.classList.remove('active');
        throw err;
    }
}

document.querySelectorAll('.admin-upload-area input[type="file"]').forEach(input => {
    input.addEventListener('change', () => {
        const label = input.closest('.admin-upload-area').querySelector('.upload-label');
        if (input.files.length > 0) label.textContent = input.files[0].name;
    });
});

// ============ UTILS ============
function generateId() {
    return Math.random().toString(36).substr(2, 9);
}

// ============ REORDER HELPER ============
async function moveItem(collection, index, direction) {
    const arr = siteData[collection];
    const newIndex = index + direction;
    
    if (newIndex < 0 || newIndex >= arr.length) return;
    
    // Swap
    const temp = arr[index];
    arr[index] = arr[newIndex];
    arr[newIndex] = temp;
    
    // Update order properties
    arr.forEach((item, i) => item.order = i);
    
    renderAll();
    await saveDataToGitHub();
}

function orderButtons(collection, index, total) {
    return \`<div class="admin-item-actions">
        <button class="admin-order-btn" onclick="moveItem('\${collection}', \${index}, -1)" \${index === 0 ? 'disabled' : ''}><i class="bi bi-chevron-up"></i></button>
        <button class="admin-order-btn" onclick="moveItem('\${collection}', \${index}, 1)" \${index === total - 1 ? 'disabled' : ''}><i class="bi bi-chevron-down"></i></button>
    </div>\`;
}

// ============ PORTFOLIO ============
function renderPfList() {
    const list = document.getElementById('pf-list');
    const arr = siteData.portfolio.sort((a, b) => (a.order ?? 9999) - (b.order ?? 9999));
    
    if (arr.length === 0) { list.innerHTML = '<div class="admin-empty">Nessun progetto aggiunto.</div>'; return; }
    
    list.innerHTML = arr.map((d, i) => \`
        <div class="admin-item">
            <img src="\${d.image}" alt="" class="admin-item-img" onerror="this.style.display='none'" />
            <div class="admin-item-info"><strong>\${d.title}</strong><span>\${d.description || ''}</span></div>
            \${orderButtons('portfolio', i, arr.length)}
            <button class="admin-remove-btn" onclick="removePf('\${d.id}')"><i class="bi bi-x-lg"></i></button>
        </div>
    \`).join('');
}

document.getElementById('pf-add').addEventListener('click', async () => {
    const title = document.getElementById('pf-title').value.trim();
    const desc = document.getElementById('pf-desc').value.trim();
    const file = document.getElementById('pf-file').files[0];
    
    if (!title || !file) { alert('Inserisci titolo e immagine.'); return; }
    
    const btn = document.getElementById('pf-add');
    btn.disabled = true; btn.textContent = 'Caricamento...';
    
    try {
        const imageUrl = await uploadFileToGitHub(file, 'portfolio', document.getElementById('pf-progress-bar'));
        
        siteData.portfolio.push({
            id: generateId(),
            title,
            description: desc,
            image: imageUrl,
            order: siteData.portfolio.length
        });
        
        await saveDataToGitHub();
        
        document.getElementById('pf-title').value = ''; 
        document.getElementById('pf-desc').value = ''; 
        document.getElementById('pf-file').value = '';
        document.querySelector('#pf-upload-area .upload-label').textContent = 'Trascina un\\'immagine o clicca per caricare';
        renderPfList();
    } catch (err) { 
        alert('Errore: ' + err.message); 
    }
    
    btn.disabled = false; btn.innerHTML = '<i class="bi bi-plus-lg"></i> Aggiungi';
});

window.removePf = async function(id) {
    if (!confirm('Rimuovere questo progetto?')) return;
    siteData.portfolio = siteData.portfolio.filter(item => item.id !== id);
    renderPfList();
    await saveDataToGitHub();
};

// ============ BRANDS ============
function renderBrList() {
    const list = document.getElementById('br-list');
    const arr = siteData.brands.sort((a, b) => (a.order ?? 9999) - (b.order ?? 9999));
    
    if (arr.length === 0) { list.innerHTML = '<div class="admin-empty">Nessun brand aggiunto.</div>'; return; }
    
    list.innerHTML = arr.map((d, i) => \`
        <div class="admin-item">
            <img src="\${d.logo}" alt="" class="admin-item-img" style="background:#222;padding:4px;" onerror="this.style.display='none'" />
            <div class="admin-item-info"><strong>\${d.name}</strong><span>\${d.link || 'Nessun link'}</span></div>
            \${orderButtons('brands', i, arr.length)}
            <button class="admin-remove-btn" onclick="removeBr('\${d.id}')"><i class="bi bi-x-lg"></i></button>
        </div>
    \`).join('');
}

document.getElementById('br-add').addEventListener('click', async () => {
    const name = document.getElementById('br-name').value.trim();
    const link = document.getElementById('br-link').value.trim();
    const file = document.getElementById('br-file').files[0];
    
    if (!name || !file) { alert('Inserisci nome e logo.'); return; }
    
    const btn = document.getElementById('br-add');
    btn.disabled = true; btn.textContent = 'Caricamento...';
    
    try {
        const logoUrl = await uploadFileToGitHub(file, 'brands', document.getElementById('br-progress-bar'));
        
        siteData.brands.push({
            id: generateId(),
            name,
            logo: logoUrl,
            link: link || '#',
            order: siteData.brands.length
        });
        
        await saveDataToGitHub();
        
        document.getElementById('br-name').value = ''; 
        document.getElementById('br-link').value = ''; 
        document.getElementById('br-file').value = '';
        document.querySelector('#br-upload-area .upload-label').textContent = 'Carica logo (SVG/PNG)';
        renderBrList();
    } catch (err) { 
        alert('Errore: ' + err.message); 
    }
    
    btn.disabled = false; btn.innerHTML = '<i class="bi bi-plus-lg"></i> Aggiungi';
});

window.removeBr = async function(id) {
    if (!confirm('Rimuovere questo brand?')) return;
    siteData.brands = siteData.brands.filter(item => item.id !== id);
    renderBrList();
    await saveDataToGitHub();
};

// ============ TESTIMONIALS ============
function renderTsList() {
    const list = document.getElementById('ts-list');
    const arr = siteData.testimonials.sort((a, b) => (a.order ?? 9999) - (b.order ?? 9999));
    
    if (arr.length === 0) { list.innerHTML = '<div class="admin-empty">Nessuna recensione aggiunta.</div>'; return; }
    
    list.innerHTML = arr.map((d, i) => \`
        <div class="admin-item">
            <img src="\${d.photo || './assets/images/people/man.jpg'}" alt="" class="admin-item-img" style="border-radius:50%;" onerror="this.style.display='none'" />
            <div class="admin-item-info"><strong>\${d.name}</strong><span>\${d.role || ''} — "\${(d.text || '').substring(0, 60)}\${(d.text || '').length > 60 ? '...' : ''}"</span></div>
            \${orderButtons('testimonials', i, arr.length)}
            <button class="admin-remove-btn" onclick="removeTs('\${d.id}')"><i class="bi bi-x-lg"></i></button>
        </div>
    \`).join('');
}

document.getElementById('ts-add').addEventListener('click', async () => {
    const name = document.getElementById('ts-name').value.trim();
    const role = document.getElementById('ts-role').value.trim();
    const text = document.getElementById('ts-text').value.trim();
    const file = document.getElementById('ts-file').files[0];
    
    if (!name || !text) { alert('Inserisci nome e testo.'); return; }
    
    const btn = document.getElementById('ts-add');
    btn.disabled = true; btn.textContent = 'Caricamento...';
    
    try {
        let photoUrl = '';
        if (file) {
            photoUrl = await uploadFileToGitHub(file, 'testimonials', document.getElementById('ts-progress-bar'));
        }
        
        siteData.testimonials.push({
            id: generateId(),
            name,
            role,
            text,
            photo: photoUrl,
            order: siteData.testimonials.length
        });
        
        await saveDataToGitHub();
        
        document.getElementById('ts-name').value = ''; 
        document.getElementById('ts-role').value = ''; 
        document.getElementById('ts-text').value = ''; 
        document.getElementById('ts-file').value = '';
        document.querySelector('#ts-upload-area .upload-label').textContent = 'Carica foto (opzionale)';
        renderTsList();
    } catch (err) { 
        alert('Errore: ' + err.message); 
    }
    
    btn.disabled = false; btn.innerHTML = '<i class="bi bi-plus-lg"></i> Aggiungi';
});

window.removeTs = async function(id) {
    if (!confirm('Rimuovere questa recensione?')) return;
    siteData.testimonials = siteData.testimonials.filter(item => item.id !== id);
    renderTsList();
    await saveDataToGitHub();
};

// ============ LOAD ALL ============
function renderAll() { 
    renderPfList(); 
    renderBrList(); 
    renderTsList(); 
}
</script>
</body>
</html>
`;

fs.writeFileSync('admin.html', html + newScript);
